<template>
  <div class="w-100">
    <a-page-header class="px-0" :title="title" sub-title="Danh sách tất cả đơn hàng"/>
    <Fillter css='d-inline-block' v-on:eventFillter="fillterRow"/>
    <div class="float-end mt-1">
      <a-button @click="submitAllStatus" v-if="!$device.isMobile">Xử lý nhanh</a-button>
...
export default {
  async asyncData({ $axios }) {
    const o = await $axios.$get("/api/oders");
  ...
  methods: {
    submitAllStatus(){
      ...
      is.$confirm({
        title: 'Bạn có chắc chắn muốn hoàn thành các hóa đơn đang xử lý ?',
        content: h => <div style="color:red;">Số lượng: {list.length}</div>,
        onOk() {
          list.forEach(e=>{
            const index = is.oders.findIndex((i) => i._id == e._id);
            is.oders[index].status = 'Hoàn thành'
            is.$axios.$patch(`/api/oders/${e._id}`, {status: 'Hoàn thành'})
            let o = is.oders.find(i => i._id == e._id)
            o.product.forEach(e=>{
              let p = is.products.find(i => i._id == e._id)
              is.$axios.$patch(`/api/products/${e._id}`, {quantity: p.quantity - e.quantity})
            })
          })
        },
        class: 'test',
    ...
    addRow(){
      this.isModal = true;
      this.isAdd = true;
      this.formModal = {'phone': '','fullname': '','type': 'Website','ship': '30000','note': ''}
      this.editData = {'user': '','items': [], 'total': 0, 'cost': 0}
    },
    deleteRow(e){
      if(e.status == 'Hoàn thành'){
        let temp = []
